...
function OnboardWizardApp(props: {
  ...
  const [runtimeMode, setRuntimeMode] = createSignal<'tmux' | 'pty-rust'>(props.initial.runtimeMode);
  ...
  const currentOptions = createMemo<SelectOption<string | boolean>[]>(() => {
    ...
    if (step === 'runtimeMode') {
      return [
        ...
        { value: 'pty-rust', label: 'pty-rust', description: 'Rust sidecar runtime mode' },
      ];
    }
  ...
  const commitSelectionStep = (): void => {
    ...
    } else if (step === 'runtimeMode' && (selected.value === 'tmux' || selected.value === 'pty-rust')) {
